...
public class KTableAggDemo {
    ...
    public static void main(String[] args) {
        ...
        streamsBuilder.table(AppConfigs.topicName,
            ...
            .aggregate(
                ...
                Materialized.<String, DepartmentAggregate, KeyValueStore<Bytes, byte[]>>as(
                    // Always make sure to provide a name to the State Store you are going to query!!!
        ...
        //To validate the Streams' application is up and running before starting the Query Server we need to subscribe to
        //a state listener
        streams.setStateListener((currentSate, prevState) -> {
            logger.info("State changing to " + currentSate + " from " + prevState);
            queryServer.setActive(currentSate == KafkaStreams.State.RUNNING && prevState == KafkaStreams.State.REBALANCING);
        });
        streams.start();
        queryServer.start();
